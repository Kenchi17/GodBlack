...
class MoodleClient(object):
    ...
    def getUserData(self):
        try:
            tokenUrl = (
                self.path
                + "login/token.php?service=moodle_mobile_app&username="
    ...
    def login(self):
        try:
            ...
            if counter > 0:
                ...
                try:
                    self.userid = soup.find("div", {"id": "nav-notification-popover-container"})[
                        "data-userid"
                    ]
                except:
                    try:
    ...
    def createEvidence(self, name, desc=""):
        ...
        saveevidence = (
            self.path
            + "admin/tool/lp/user_evidence_edit.php?id=&userid="
            + self.userid
            + "&return="
        )
        payload = {
    ...
    def saveEvidence(self, evidence):
        evidenceurl = (
            self.path
            + "admin/tool/lp/user_evidence_edit.php?id="
            + evidence["id"]
            + "&userid="
            + self.userid
            + "&return=list"
        ...
        for n in nodes:
            ...
            for f in nodefiles:
                ...
                try:
                    ...
                    directurl = str(directurl).replace(
    ...
    def deleteEvidence(self, evidence):
        ...
        savejson = [
            ...
            {
                "index": 1,
                "methodname": "tool_lp_data_for_user_evidence_list_page",
    ...
    async def upload_file(self, file, evidence=None, itemid=None, progressfunc=None, args=()):
        try:
            ...
            upload_data = {
                ...
                "areamaxbytes": (None, "4294967296"),  # Hack para aulacened
    ...
    async def upload_file_blog(self, file, blog=None, itemid=None, progressfunc=None, args=()):
        try:
            ...
            if self.userdata:
                if "token" in self.userdata:
                    data["url"] = (
                        str(data["url"]).replace("pluginfile.php/", "webservice/pluginfile.php/")
                        + "?token="
                        + self.userdata["token"]
                    )
            return itempostid, data
    ...
    async def upload_file_draft(self, file, progressfunc=None, args=()):
        ...
        usertext = "ObisoftDev"
        query = self.extractQuery(soup.find("object", attrs={"type": "text/html"})["data"])
        client_id = str(soup.find("div", {"class": "filemanager"})["id"]).replace(
            "filemanager-", ""
        )
        ...
        upload_file = {"repo_upload_file": (file, of, "application/octet-stream"), **upload_data}
        post_file_url = self.path + "repository/repository_ajax.php?action=upload"
        encoder = rt.MultipartEncoder(upload_file, boundary=b)
        progrescall = CallingUpload(progressfunc, file, args)
        callback = partial(progrescall)
    ...
    def delteFile(self, name):
        ...
        _qf__core_user_form_private_files = soup.find(
            "input", {"name": "_qf__core_user_form_private_files"}
        )["value"]
        ...
        # save file
        saveUrl = (
            self.path + "lib/ajax/service.php?sesskey=" + sesskey + "&info=core_form_dynamic_form"
        )
        savejson = [
            {
                "index": 0,
                ...
                "args": {
                    "formdata": "sesskey="
                    + sesskey
                    + "&_qf__core_user_form_private_files="
                    + _qf__core_user_form_private_files
                    + "&files_filemanager="
                    + query["itemid"]
                    + "",
